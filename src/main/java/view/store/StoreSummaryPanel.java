...
public class StoreSummaryPanel extends JPanel {
    ...
    public StoreSummaryPanel(String id, String name, String imagePath) {
        Box box = Box.createVerticalBox();
        box.add(Box.createVerticalGlue());

        setPreferredSize(new Dimension(300, 300));
        JLabel imageLabel = new StoreImage(new ImageIcon(getClass().getClassLoader().getResource(imagePath)));
        imageLabel.addMouseListener(new StoreClickListner(id));
        box.add(imageLabel);

        JLabel nameLabel = new JLabel(name);
        nameLabel.setFont(new Font("Arial", Font.BOLD, 20));
        nameLabel.addMouseListener(new StoreClickListner(id));
        box.add(nameLabel);
